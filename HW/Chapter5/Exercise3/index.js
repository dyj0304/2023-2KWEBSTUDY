...
app.get('/fare', async (req, res, next) => {
    try {
        ...
        'Sum(Round(0.001 * types.fare_rate * trains.distance, -2)) AS totalFare ' +
            'FROM tickets ' +
        ...
        return res.send(`Total fare of ${name} is ${totalFare}.`)
    }
    catch (err) {
        console.error(err);
...
app.get('/train/status', async (req, res, next) => {
    try {
        const { tid } = req.query;
        const sql = 'SELECT Count(seat_number) AS occupied, max_seats AS maximum ' +
            'FROM tickets ' +
            'INNER JOIN trains ON tickets.train = trains.id AND trains.id = ? ' +
            'INNER JOIN types ON types.id = trains.type';
        const { occupied, maximum } = (await runQuery(sql, [tid]))[0];
        const seatLeft = occupied < maximum;
        return res.send(`Train ${tid} is ${seatLeft ? 'not ' : ''}sold out`);
    }
    catch (err) {
        console.error(err);
        return res.sendStatus(500);
